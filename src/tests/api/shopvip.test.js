...
describe('shopvip API', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error('Error setting up auth for shopvip tests:', error.message);
    }
  });

  describe('POST /shopvip/create', () => {
    it('should create a new shopvip', async () => {
      const testCases = [
        {
          name: 'Valid shopvip creation',
          expectedStatus: [200, 201, 500],
          headers: authHeaders,
          body: { name: 'Test shopvip', description: 'Test shopvip description' }
        },
        {
      ...
      const results = await testEndpoint(app, 'POST', '/shopvip/create', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });

  describe('GET /shopvip/read/:id', () => {
    it('should read a shopvip by id', async () => {
      const testCases = [
      ...
      const results = await testEndpoint(app, 'GET', '/shopvip/read/507f1f77bcf86cd799439011', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });
