...
describe('land API', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error('Error setting up auth for land tests:', error.message);
    }
  });

  describe('POST /land/create', () => {
    it('should create a new land', async () => {
      const testCases = [
        {
          name: 'Valid land creation',
          expectedStatus: [200, 201, 500],
          headers: authHeaders,
          body: { name: 'Test land', description: 'Test land description' }
        },
        {
      ...
      const results = await testEndpoint(app, 'POST', '/land/create', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });

  describe('GET /land/read/:id', () => {
    it('should read a land by id', async () => {
      const testCases = [
      ...
      const results = await testEndpoint(app, 'GET', '/land/read/507f1f77bcf86cd799439011', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });
