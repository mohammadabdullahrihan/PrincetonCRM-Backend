...
describe('landbuyer API', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error('Error setting up auth for landbuyer tests:', error.message);
    }
  });

  describe('POST /landbuyer/create', () => {
    it('should create a new landbuyer', async () => {
      const testCases = [
        {
          name: 'Valid landbuyer creation',
          expectedStatus: [200, 201, 500],
          headers: authHeaders,
          body: { name: 'Test landbuyer', description: 'Test landbuyer description' }
        },
        {
      ...
      const results = await testEndpoint(app, 'POST', '/landbuyer/create', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });

  describe('GET /landbuyer/read/:id', () => {
    it('should read a landbuyer by id', async () => {
      const testCases = [
      ...
      const results = await testEndpoint(app, 'GET', '/landbuyer/read/507f1f77bcf86cd799439011', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });
