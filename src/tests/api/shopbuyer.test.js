...
describe('shopbuyer API', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error('Error setting up auth for shopbuyer tests:', error.message);
    }
  });

  describe('POST /shopbuyer/create', () => {
    it('should create a new shopbuyer', async () => {
      const testCases = [
        {
          name: 'Valid shopbuyer creation',
          expectedStatus: [200, 201, 500],
          headers: authHeaders,
          body: { name: 'Test shopbuyer', description: 'Test shopbuyer description' }
        },
        {
      ...
      const results = await testEndpoint(app, 'POST', '/shopbuyer/create', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });

  describe('GET /shopbuyer/read/:id', () => {
    it('should read a shopbuyer by id', async () => {
      const testCases = [
      ...
      const results = await testEndpoint(app, 'GET', '/shopbuyer/read/507f1f77bcf86cd799439011', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });
