...
describe('shop API', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      console.error('Error setting up auth for shop tests:', error.message);
    }
  });
  ...
  describe('POST /shop/create', () => {
    it('should create a new shop', async () => {
      const testCases = [
        {
          name: 'Valid shop creation',
          expectedStatus: [200, 201, 500],
          headers: authHeaders,
          body: { name: 'Test shop', description: 'Test shop description' }
        },
        {
      ...
      const results = await testEndpoint(app, 'POST', '/shop/create', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });

  describe('GET /shop/read/:id', () => {
    it('should read a shop by id', async () => {
      const testCases = [
      ...
      const results = await testEndpoint(app, 'GET', '/shop/read/507f1f77bcf86cd799439011', testCases);
      results.forEach(result => expect(result.passed).toBe(true));
    });
  });
